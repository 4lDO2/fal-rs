...
impl BlockGroupDescriptor {
    ...
    pub fn unalloc_block_count(&self) -> u32 {
        u32::from(self.base.unalloc_block_count_lo)
            | self
                .ext
                .as_ref()
                .map(|ext| (u32::from(ext.unalloc_block_count_hi) << 16))
                .unwrap_or(0)
    }
    pub fn unalloc_inode_count(&self) -> u32 {
        u32::from(self.base.unalloc_inode_count_lo)
            | self
                .ext
                .as_ref()
                .map(|ext| (u32::from(ext.unalloc_inode_count_hi) << 16))
                .unwrap_or(0)
    }
    ...
    pub fn block_bm_checksum(&self) -> u32 {
        u32::from(self.base.block_bm_csum_lo)
            | self
                .ext
                .as_ref()
                .map(|ext| (u32::from(ext.block_bm_csum_hi) << 16))
                .unwrap_or(0)
    }
    ...
    pub fn inode_bm_checksum(&self) -> u32 {
        u32::from(self.base.inode_bm_csum_lo)
            | self
                .ext
                .as_ref()
                .map(|ext| (u32::from(ext.inode_bm_csum_hi) << 16))
                .unwrap_or(0)
    }
    ...
    pub fn parse(bytes: &[u8], superblock: &Superblock, index: u32) -> Result<Self, scroll::Error> {
        ...
        if BlockGroupDescriptor::calculate_crc32c(superblock.checksum_seed().unwrap(), index, bytes)
            as u16
            != this.base.csum
        {
            return Err(scroll::Error::BadInput {
                size: 2,
                msg: "block group descriptor checksum mismatch",
            });
        }
        ...
        if BlockGroupDescriptorFlags::from_bits(this.base.flags).is_none() {
            return Err(scroll::Error::BadInput {
                size: 2,
                msg: "invalid block group flags bitmask",
            });
        }
...
mod bgd_err {
    ...
    pub enum BgdError {
        ...
        #[error("block group descriptor parsing error: {0}")]
        ParseError(#[from] scroll::Error),

        #[error("invalid block group descriptor checksum")]
        ChecksumMismatch,
    }
}
...
pub fn store_block_group_descriptor<D: fal::Device>(
    ...
    let (descriptor_bytes, absolute_offset) =
        get_block_group_descriptor(filesystem, index, &mut block_bytes)?;
    BlockGroupDescriptor::serialize(
        bgdesc,
        filesystem.superblock.checksum_seed().unwrap(),
        index as u32,
        descriptor_bytes,
    ...
    if filesystem.superblock.has_metadata_checksums() {
        bgdesc.base.csum = BlockGroupDescriptor::calculate_crc32c(
            filesystem.superblock.checksum_seed().unwrap(),
            index as u32,
            descriptor_bytes,
        ) as u16;
    ...
    filesystem.disk.write_block_raw(
        filesystem,
        BlockKind::Metadata,
        absolute_offset,
        &block_bytes,
...
pub fn inode_exists<D: fal::DeviceRo>(
    ...
    if descriptor
        .flags()
        .contains(BlockGroupDescriptorFlags::INODE_UNINIT)
    {
        return Ok(false);
    }
    ...
    filesystem.disk.read_block(
        filesystem,
        BlockKind::Metadata,
        bm_start_baddr + u64::from(bm_block_index),
        &mut block_bytes,
    )?;

    let byte_index_inside_bm = u32::try_from(
        u64::from(index_inside_group) / u64::from(filesystem.superblock.block_size()),
    )
    .unwrap();
...
impl<'a, 'b> Bitmap<'a, 'b> {
    ...
    pub fn set(&mut self, baddr: u64, value: bool) {
        self.set_rel(
            (baddr % u64::from(self.superblock.blocks_per_group))
                .try_into()
                .unwrap(),
            value,
        )
    ...
    pub fn free_blocks_after_start_rel(&self, start: u32, max_bytes_to_read: usize) -> u32 {
        ...
        for mut byte in self
            .bytes
            .iter()
            .copied()
            .skip(start as usize / 8)
            .take(max_bytes_to_read)
        {
            if byte == 0x00 {
                count += 8;
            } else {
...
pub fn read_block_bitmap<'a, 'b, D: fal::DeviceRo>(
    ...
    if descriptor
        .flags()
        .contains(BlockGroupDescriptorFlags::BLOCK_BITMAP_UNINIT)
    {
        return Ok(None);
    }
...
pub fn set_block_exists<D: fal::Device>(
    ...
    filesystem
        .disk
        .write_block_raw(filesystem, BlockKind::Metadata, baddr, &block_bytes)?;
    Ok(())
}
...
>(
    ...
    let bgdt_block_count =
        block_group_count as u32 * filesystem.superblock.block_size() / size as u32;
    ...
    for rel_baddr in 0..bgdt_block_count {
        filesystem.disk.read_block_raw(
            filesystem,
            BlockKind::Metadata,
            bgdt_first_block + u64::from(rel_baddr),
            &mut block_bytes,
        )?;
        for rel_index in 0..descs_per_block {
            ...
            let mut desc =
                BlockGroupDescriptor::parse(desc_bytes, &filesystem.superblock, abs_index)?;
    ...
    match iter_through_bgdescs(
        ...
        |handle| -> Result<Option<Range<u64>>, AllocateBlockError> {
            ...
            if unalloc_count >= len {
                log::trace!(
                    "block group {bg} had {free} free {word}s",
                    bg = handle.abs_index,
                    free = handle.desc.unalloc_block_count(),
                    word = word
                );
                ...
                let mut bitmap = match {
                    match thing_to_allocate {
                        ThingToAllocate::Blocks => {
                            handle
                                .desc
                                .block_bitmap(filesystem, &mut bitmap_block_bytes, 0)?
                        }
                        ThingToAllocate::Inodes => {
                            handle
                                .desc
                                .inode_bitmap(filesystem, &mut bitmap_block_bytes, 0)?
                        }
                    }
                } {
                    Some(bm) => bm,
                ...
                // TODO
                let available_blocks_after =
                    bitmap.free_blocks_after_start_rel(first_unused_rel_baddr, 0xFFFF_FFFF);
                    ...
                    match thing_to_allocate {
                        ThingToAllocate::Blocks => {
                            ...
                            filesystem
                                .info
                                .free_blocks
                                .fetch_sub(u64::from(len), atomic::Ordering::Acquire);
                            handle
                                .desc
                                .set_unalloc_block_count(handle.desc.unalloc_block_count() - len)
                        }
                        ThingToAllocate::Inodes => {
                            ...
                            filesystem
                                .info
                                .free_inodes
                                .fetch_sub(len, atomic::Ordering::Acquire);
                            handle
                                .desc
                                .set_unalloc_inode_count(handle.desc.unalloc_inode_count() - len)
                        }
                    }
                    ...
                    #[cfg(debug_assertions)]
                    {
                        for block in abs_block_range.clone() {
                            if !block_exists(block, filesystem).unwrap() {
                                log::error!(
                                    "{word} {addr} didn't exist, after it was allocated",
                                    word = word,
                                    addr = block
                                );
                            }
