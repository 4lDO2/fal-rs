...
//! __ NOTE: Every field here is stored as _big endian_ __
...
impl fmt::Debug for JournalSuperblockV2 {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("JournalSuperblockV2")
            .field(
                "compat_features",
                &JournalCompatFeatureFlags::from_bits_truncate(self.compat_features),
            )
            .field(
                "incompat_features",
                &JournalIncompatFeatureFlags::from_bits_truncate(self.compat_features),
            )
            ...
            .field(
                "checksum_ty",
                &JournalChecksumType::from_raw(self.checksum_ty),
            )
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, scroll::Error> {
        ...
        if v1.header.magic != JOURNAL_HEADER_MAGIC {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "the journal superblock's magic number didn't match",
            });
        }
        ...
        let v2 = if v1.header.block_ty == BlockType::SuperblockV2 as u32 {
            ...
            let checksum_ty = match JournalChecksumType::from_raw(v2.checksum_ty) {
                ...
                None => {
                    return Err(scroll::Error::BadInput {
                        size: 4,
                        msg: "invalid journal superblock checksum type",
                    })
                }
            };
            ...
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "invalid journal superblock block type, expected superblock v1 or v2",
            });
        };

        Ok(Self { base: v1, ext: v2 })
    }
    pub fn seed(&self) -> u32 {
        calculate_crc32c(!0, &self.ext.unwrap().uuid)
    }
    pub fn checksum_ty(&self) -> Option<JournalChecksumType> {
        self.ext.as_ref().map(|ext| ext.checksum_ty).map(|raw| {
            JournalChecksumType::from_raw(raw).expect("Somehow the init check was bypassed")
        })
    }
...
impl JournalBlockTag {
    pub fn parse(
        bytes: &[u8],
        offset: &mut usize,
        incompat_flags: JournalIncompatFeatureFlags,
        journal_uuid: [u8; 16],
        ...
        let wrapped_flags = match JournalTagFlags::from_bits(flags) {
            ...
            None => {
                return Err(scroll::Error::BadInput {
                    size: 4,
                    msg: "the journal block tag flags contained unknown flags",
                })
            }
        };
        ...
        if !incompat_flags.contains(JournalIncompatFeatureFlags::_64_BIT) && data_block_hi != 0 {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "nonzero high data block address without the 64 bit journal feature",
            });
        }
    ...
    pub fn tag_size(incompat_flags: JournalIncompatFeatureFlags) -> usize {
        ...
        12 + if incompat_flags.contains(JournalIncompatFeatureFlags::V2_CHECKSUM) {
            2
        } else {
            0
        } - if incompat_flags.contains(JournalIncompatFeatureFlags::_64_BIT) {
            4
        } else {
            0
        }
...
impl JournalDescriptorBlock {
    ...
    pub fn parse(
        ...
        if header.block_ty != BlockType::Descriptor as u32 {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "journal descriptor block wasn't a descriptor",
            });
        }
        if header.magic != JOURNAL_HEADER_MAGIC {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "the journal descriptor's magic number didn't match",
            });
        }
        ...
        let tags = (0..(superblock.block_size as usize - 12) / tag_size)
            .map(|_| {
                JournalBlockTag::parse(&bytes, &mut offset, superblock.incompat_features(), uuid)
            })
            .take_while(Result::is_ok)
            .collect::<Result<Vec<_>, _>>()
            .expect("logic error");
        let tail: JournalDescriptorBlockTail =
            bytes.pread_with(superblock.block_size as usize - 4, scroll::BE)?;

        if tail.checksum != Self::calculate_crc32c(superblock.seed(), bytes) {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "journal descriptor block checksum didn't match",
            });
        }

        Ok(Self { header, tags, tail })
    }
}
...
impl JournalCommitBlock {
    ...
    pub fn parse(seed: u32, bytes: &[u8]) -> Result<Self, scroll::Error> {
        ...
        if this.header.magic != JOURNAL_HEADER_MAGIC {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "the journal commit block's magic number didn't match",
            });
        }
        ...
        if this.header.block_ty != BlockType::BlockCommitRecord as u32 {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "the journal commit block header didn't have the type 'block commit header'",
            });
        }
        ...
        if this.checksum[0] != Self::calculate_crc32c(seed, bytes) {
            return Err(scroll::Error::BadInput {
                size: 4,
                msg: "checksum mismatch in the journal commit block",
            });
        }
...
pub enum JournalInitError {
    #[error("i/o error: {0}")]
    IoError(#[from] crate::inode::InodeIoError),

    #[error("journal doesn't support v3 checksums")]
...
pub fn find_transaction<D: fal::DeviceRo>(
    ...
    while let Some((meta_baddr, header)) = find_meta_block(
        filesystem,
        journal_inode,
        start_block,
        block_bytes,
        ...
        if header.block_ty == BlockType::Descriptor as u32
            || header.block_ty == BlockType::BlockCommitRecord as u32
        {
        ...
        if header.block_ty == BlockType::Descriptor as u32 {
            ...
            let new_desc_block = JournalDescriptorBlock::parse(
                block_bytes,
                journal_superblock,
                journal_superblock.uuid(&filesystem.superblock),
            )?;
...
pub enum VerifyTransactionError {
    #[error("i/o error: {0}")]
    IoErr(#[from] crate::inode::InodeIoError),

    #[error("checksums mismatch")]
    ChecksumMismatch,
}
...
pub fn verify_transaction<D: fal::DeviceRo>(
    ...
    for (desc, data_block_range) in &transaction.desc_blocks {
        for (tag, data_baddr) in desc.tags.iter().zip(data_block_range.clone()) {
            journal_inode.read_block(data_baddr, filesystem, &mut data_block)?;
            if tag.checksum
                != calculate_tag_crc32(
                    journal_superblock.seed(),
                    transaction.commit_block.header.xid,
                    &data_block,
                )
            {
...
impl Journal {
    pub fn load<D: fal::Device>(
        ...
        if !superblock
            .incompat_features()
            .contains(JournalIncompatFeatureFlags::V3_CHECKSUM)
        {
        ...
        while let Some((next_transacton_baddr, transaction)) = find_transaction(
            filesystem,
            &inode,
            current_baddr,
            &mut block_bytes,
